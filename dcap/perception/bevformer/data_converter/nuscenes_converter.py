...
def _fill_trainval_infos(nusc,
    ...
    for sample in track_iter_progress(nusc.sample):
        ...
        if not test:
            ...
            # 过滤异常的GT boxes (尺寸为0、负数等)
            w, l, h = dims[:, 0], dims[:, 1], dims[:, 2]
            valid_size_mask = (w > 0) & (l > 0) & (h > 0)
            invalid_count = np.sum(~valid_size_mask)
            
            if invalid_count > 0:
                print(f"Sample {sample['token']}: 发现 {invalid_count} 个异常GT boxes, 已过滤")
                print(f"  异常尺寸: w_min={w.min():.6f}, l_min={l.min():.6f}, h_min={h.min():.6f}")
                
                # 只保留有效的boxes
                gt_boxes = gt_boxes[valid_size_mask]
                names = names[valid_size_mask]
                velocity = velocity[valid_size_mask]
                valid_flag = valid_flag[valid_size_mask]
                annotations = [annotations[i] for i in range(len(annotations)) if valid_size_mask[i]]
