...
if __name__ == '__main__':
    nusc = NuScenes(version='v1.0-mini', dataroot='data/stt4at', verbose=True)
    # render_annotation('7603b030b42a4b1caa8c443ccc1a7d52')
    
    # Load BEVFormer detection results.
    bevformer_results = mmcv.fileio.io.load('test/bevformer_base/Mon_Oct_27_02_08_24_2025/pts_bbox/results_nusc.json')
    ...
    for id in range(len(sample_token_list)):
        print(f"Processing sample {id+1}/{len(sample_token_list)}: {sample_token_list[id]}")
        safe_token = sample_token_list[id].replace('/', '_')
        render_sample_data(sample_token_list[id], pred_data=bevformer_results, out_path=f"vis_output/vis_output_{safe_token}")
