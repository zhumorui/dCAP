...
class NuScenesMap:
    ...
    def __init__(self,
        ...
        # uniad-These are the non-geometric layers which have polygons as the geometric descriptors.
        # self.non_geometric_polygon_layers = ['drivable_area', 'road_segment', 'road_block', 'lane', 'ped_crossing',
        #                                      'walkway', 'stop_line', 'carpark_area']

        # just have road_divider, lane_divider and drivable_area
        self.non_geometric_polygon_layers = ['drivable_area', 'lane']

        # uniad-We want to be able to search for lane connectors, but not render them.
        ...
        # uniad-These are the non-geometric layers which have line strings as the geometric descriptors.
        ...
        for layer_name in self.non_geometric_polygon_layers:
            if layer_name == 'drivable_area':  # Drivable area has more than one geometric representation.
                ...
                for record in self.__dict__[layer_name]:
                    polygon_obj = self.get('polygon', record['polygon_tokens'])
...
class NuScenesMapExplorer:
    ...
    def get_map_mask(self,
        ...
        if self.map_api.map_name == 'Town01':
        ...
        elif self.map_api.map_name == 'Town04':
            ...
            print(f'Error: Invalid map name {self.map_api.map_name}!')
    ...
    def get_map_mask_in_image(self,
        ...
        for layer_name in layer_names:
            if layer_name in self.map_api.non_geometric_line_layers:
                ...
                for token in records_in_patch[layer_name]:
                    record = self.map_api.get(layer_name, token)
                    polygon = self.map_api.extract_polygon(record['polygon_tokens'])
                    if polygon.is_valid:
    ...
    def extract_polygon(self, polygon_token: str) -> Polygon:
        ...
        #TODO: fix the issue with the polygon nodes being less than 3
        if len(exterior_coords) < 3:
            # raise ValueError(f"Polygon requires at least three points, but got: {exterior_coords}")
            exterior_coords = exterior_coords + exterior_coords + exterior_coords
    ...
    def _is_polygon_record_in_patch(self,
        ...
        if layer_name == 'drivable_area':
            ...
            geom = self.map_api.extract_polygon(record['polygon_tokens'])
    ...
    def mask_for_polygons(polygons: MultiPolygon, mask: np.ndarray) -> np.ndarray:
        ...
        if isinstance(polygons, MultiPolygon):
            ...
            exteriors = [int_coords(polygons.exterior.coords)]

        if isinstance(polygons, MultiPolygon):
            interiors = [int_coords(pi.coords) for poly in polygons.geoms for pi in poly.interiors]
        else:
            interiors = [int_coords(pi.coords) for pi in polygons.interiors]
