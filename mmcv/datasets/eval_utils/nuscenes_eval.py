...
class NuScenesEval_custom(NuScenesEval):
    ...
    def _sanitize_gt_boxes(self, eps: float = 1e-3):
        """Clamp zero/negative GT box sizes to avoid nuScenes eval assertions."""
        invalid_total = 0
        for sample_token, boxes in self.gt_boxes.boxes.items():
            for box in boxes:
                ...
                if (size <= 0).any():
                    ...
                    box.size = size.tolist()
        if invalid_total > 0:
            print(f'[NuScenesEval_custom] Detected {invalid_total} GT boxes with non-positive size; '
                  f'clamped to >= {eps} m for evaluation stability.')
