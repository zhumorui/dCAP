...
class NuScenesE2EDataset(NuScenesDataset):
    ...
    def get_ann_info(self, index):
        ...
        if 'gt_inds' in info:
            ...
            gt_inds = self._compute_gt_inds_from_ann_tokens(ann_tokens)
    ...
    def _compute_gt_inds_from_ann_tokens(self, ann_tokens):
        """Build instance indices on-the-fly when infos miss gt_inds."""
        instance_inds = []
        for ann_token in ann_tokens:
            ann_info = self.nusc.get('sample_annotation', ann_token)
            instance_inds.append(
                self.nusc.getind('instance', ann_info['instance_token']))
        return np.array(instance_inds)
    ...
    def occ_get_detection_ann_info(self, index):
        ...
        if 'gt_inds' in info:
            ...
            sample = self.nusc.get('sample', info['token'])
            gt_ins_inds = self._compute_gt_inds_from_ann_tokens(
                np.array(sample['anns']))
